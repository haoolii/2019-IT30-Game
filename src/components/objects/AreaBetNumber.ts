...
export default class AreaBetNumber extends WrapperContainer {
  ...
  private displayNumber(number: number) {
    ...
    numberList.map((num: string, i: number) => {
      ...
      switch (num) {
        case '.':
          _sprite = new Sprite(imagePath.tablePath, `areabetdot`)
          break
        case ',':
          _sprite = new Sprite(imagePath.tablePath, `areabetcomma`)
          _sprite.y = 10
          break
        case '/':
          _sprite = new Sprite(imagePath.tablePath, `areabetslash`)
          break
        case 'e':
          _sprite = new Sprite(imagePath.tablePath, `areabete`)
          _sprite.y = -2
          w = 30
          break
        case 'w':
          _sprite = new Sprite(imagePath.tablePath, `areabetw`)
          _sprite.y = -2
          _sprite.x = 2
          w = 25
          break
        default:
          _sprite = new Sprite(imagePath.tablePath, `areabet${num}`)
          break
      }
